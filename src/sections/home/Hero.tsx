...
import { scaleAnimVariants } from "@/utils/FramerVariants";
...
const Hero = ({ heading, gradientText, btnText, description, className, ...props }: props) => {
  return (
    <section
      className={cn(
        "relative  mx-auto mb-[95px] flex w-full items-center justify-between gap-10 [--top-space:160px] max-lg:flex-wrap max-lg:text-center max-md:max-w-[95%] max-md:text-center md:mb-[112px] lg:mb-[144px] lg:px-12 xl:max-w-[1400px]",
        className
      )}>
        ...
        <h1 className='relative w-full  flex-wrap  overflow-hidden  text-[32px] font-semibold text-dark selection:bg-fuchsia-700 selection:text-white dark:text-white max-lg:text-center max-md:text-center xl:text-p64'>
          {heading.split(",").map((word, index) => (
            <span key={index} className={cn("inline-flex overflow-hidden brightness-[1.8] dark:brightness-200 max-lg:text-center")}>
              <span
                className={cn(
                  "inline-flex bg-clip-text transition-[color,opacity,background-image]",
                  word === " " && "[width:0.25ch]",
                  index === 0 ? "color-animation-1" : "color-animation-2"
                )}>
                {`${word} `}&nbsp;
              </span>
            </span>
          ))}
          <div className='slide-right absolute bottom-0 h-1 w-[80%] -translate-x-full rounded-full bg-gradient-to-r  from-transparent via-fuchsia-700 via-50% to-sky-800  brightness-150 dark:via-fuchsia-500 dark:to-sky-500  dark:brightness-200' />
        </h1>
        {description && (
          <p className='md:hero-btn-animation mt-5 max-w-[700px] text-basic text-primary-dark [--delay:1.7s] dark:text-gray max-lg:mx-auto'>
            {description.split(" ").map((word, index) => (
              <span key={index} className={cn("inline-flex   overflow-hidden")}>
                <span className={cn("inline-flex", word === " " && "[width:0.5ch]")}>{`${word}`}&nbsp;</span>
              </span>
            ))}
          </p>
        )}
...
// 	)}>
// 	<div className=' mr-auto mt-[50px] w-full min-w-[300px] basis-full md:mt-[100px]  lg:ml-10 lg:mt-[var(--top-space)] '>
// 		<h1 className='relative w-full  flex-wrap  overflow-hidden  text-[40px] font-semibold text-dark selection:bg-fuchsia-500 selection:text-white dark:text-white max-lg:text-center max-md:text-center xl:text-p64'>
// 			{heading.split(",").map((word, index) => (
// 				<MotionComponent
// 					as='span'
...
// 						className={cn(
// 							"inline-flex bg-clip-text transition-all",
// 							word === " " && "[width:0.25ch]",
// 							index === 0 ? "color-animation-1" : "color-animation-2"
// 						)}>
// 						{`${word} `}&nbsp;
