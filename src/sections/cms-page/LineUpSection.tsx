...
import { CMS_DETAILS } from "@/utils/cms-data";
	...
	return (
		<section className='mx-auto mb-16 grid w-full max-w-[90%] items-center justify-center justify-items-center overflow-x-hidden py-16 text-center'>
						...
						{CMS_DETAILS.map((detail, index) => (
							<button
								...
								className={cn(
									...
									detail.name === activeCMS.name && "lg:pink-gradient max-lg:rounded-none max-lg:border-b-2 max-lg:border-b-pink-500"
								)}>
								{detail.name}
							</button>
						))}
			...
			<div
				...
				<DarkLightImage
					DarkVisibleImage={activeCMS.darkImage}
					LightVisibleImage={activeCMS.lightImage}
					alt={activeCMS.name}
					className='h-[65px] w-[65px] max-lg:mx-auto'
				/>
				<p className='text-center text-[15px]/[30px] font-350 dark:text-white  md:text-[16px]/[35px] lg:mr-16 '>{activeCMS.details}</p>
			</div>
		</section>
	);
};
