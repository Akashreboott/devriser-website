...
const QuestionsSection = ({ QUESTIONS, ...props }: props) => {
  ...
  let isLastQuestion = currentQuestion === noOfQuestions;
  let isFirstQuestion = currentQuestion === 0;
  let question = QUESTIONS[currentQuestion];
  let isAnswerSelected = answersSet.has(question.title);
          ...
          <AnimatePresence mode='wait' initial={false}>
            {!submitted && (
              <Question
                ...
                // @ts-ignore
                selected={isAnswerSelected ? answers[question.title] : ""}
              />
            )}
      ...
      {!submitted && (
        <div className=' mt-4 flex w-[min(100%,800px)] items-center justify-between [&>*]:mx-6'>
          <button
            ...
            className={cn(
              "glass mb-4 mt-2 appearance-none rounded-md bg-gradient-to-br from-light/80 to-gray/10 px-6 py-2 font-medium outline-1 outline-offset-2 focus-within:outline  focus-within:outline-dark/50 dark:from-dark/70 dark:to-dark/30 dark:focus-within:outline-white/80  lg:mt-10",
              isFirstQuestion ? "opacity-40" : "opacity-100 "
            )}>
            Prev
          </button>
          <button
            onClick={isAnswerSelected ? (!isLastQuestion ? nextQuestion : handleSubmit) : showMessage}
            className={cn(
              "glass mb-4 mt-2 appearance-none rounded-md bg-gradient-to-br from-light/80 to-gray/10 px-6 py-2 font-medium outline-1 outline-offset-2 focus-within:outline  focus-within:outline-dark/50 dark:from-dark/70 dark:to-dark/30 dark:focus-within:outline-white/80  lg:mt-10",
              !isAnswerSelected ? "opacity-40" : "opacity-100 "
            )}>
