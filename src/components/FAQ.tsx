	...
	return (
		<div className='relative grid w-full rounded-md px-4 py-2 '>
			...
			<label
				...
				className='flex w-full cursor-pointer select-none items-center justify-between gap-2 py-2 text-left  text-sm font-normal md:text-lg  peer-checked:[&>span+span]:!inline-block peer-checked:[&>span:first-of-type]:!hidden'>
				{props.question}
			...
			<div className='grid grid-rows-[0fr] transition-[grid-template-rows] duration-300 peer-checked:[grid-template-rows:1.2fr]'>
				<p className='overflow-hidden text-left text-sm   font-350  opacity-75 max-md:leading-6 md:text-lg'>{props.answer}</p>
			</div>
			<div
